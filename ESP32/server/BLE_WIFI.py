...
class ESP32_BLE():
    def __init__(self, name):
        ...
        self.ble.gatt_write(self.rx, bytes(100))  # 默认蓝牙只接收20字节，这里更改为接收100字节
    ...
    def wifi_get_data(self):
        if len(BLE_MSG) > 0:
            ...
            if wifi_connect_ok:
                # 将WiFi信息写成字典格式
                config = dict(ssid=ssid[4:], password=password)
...
if __name__ == "__main__":
    try:
        ...
        wifi_connect_ok = wifi_connect(
            config['ssid'], config['password'])  # 进行WiFi连接
        if wifi_connect_ok:
            wifi_connect_flag = 1
        else:
